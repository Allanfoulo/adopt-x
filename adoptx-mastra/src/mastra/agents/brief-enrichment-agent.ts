...
export const briefEnrichmentAgent = new Agent({
  ...
  description:
    "Generates evidence-backed analyst brief sections from verified adoption deal sources.",
...
Return only one valid JSON object with exactly these keys: executiveSummary, transactionOverview, strategicRationale, risks, marketImplications, keyTakeaways, dealStructure, confidenceScore, evidenceUsed, last30daysUsed, thesisMap, analysis.
...
Write detailed investment-banking prose, not a short summary. Generate at least two strategic rationale points, two risk analyses with mitigations, three key takeaways, two second-order effects, and cite source headlines or URLs in evidenceUsed. Return valueDrivers as objects with a concise driver title and an evidence-backed detail explaining why that driver creates value in this specific case; do not return value-driver labels alone. Every opportunity and thesis must be labeled with a confidence level in the analysis object. Build thesisMap with claim-level evidence references, a counter-thesis, invalidation conditions, implications, followTheMoney, opportunities, confidence rationale, and limitations. If nested corroborationEvidence is supplied, cite its exact externalId in thesisMap.evidenceClaims; do not invent IDs. If evidence is insufficient, use precise language such as "No defensible opportunity identified from the available evidence" or "Preliminary thesis only; confidence is low because evidence is limited." Never write "I can't think of". Set last30daysUsed to true only when the last30days tool was actually called and returned usable context.
`,
