...
function BriefReaderContent({ detail }: { detail: ArchiveDetail }) {
  ...
  return (
    <div className="space-y-10">
      ...
      <ReaderSection id="Capability Purchased" title="Capability Purchased"><ReaderBullets items={analysis?.capabilityPurchased} /></ReaderSection>
      ...
      <ReaderSection id="Value Drivers" title="Value Drivers"><ReaderBullets items={analysis?.valueDrivers} /></ReaderSection>
      <ReaderSection id="Synergy Map" title="Synergy Map"><div className="grid gap-3 md:grid-cols-3">{(analysis?.synergyMap ?? []).map((item) => <ReaderCard key={item.category} title={item.category} detail={item.items.join(" ")} />)}</div></ReaderSection>
      <ReaderSection id="Risks & Mitigations" title="Risks & Mitigations"><div className="space-y-3">{(analysis?.riskAnalysis ?? []).map((item) => <ReaderCard key={`${item.category}-${item.title}`} title={`${item.category}: ${item.title}`} detail={`${item.detail} Mitigation: ${item.mitigation}`} />)}</div></ReaderSection>
      <ReaderSection id="Market Signal" title="Market Signal"><p>{analysis?.marketSignal ?? fallback()}</p></ReaderSection>
      <ReaderSection id="Follow the Money" title="Follow the Money"><ReaderPoints items={analysis?.followTheMoney} /></ReaderSection>
      <ReaderSection id="Second-Order Effects" title="Second-Order Effects"><ReaderPoints items={(analysis?.secondOrderEffects ?? []).map((item) => ({ title: item.question, detail: item.answer }))} /></ReaderSection>
      ...
      <ReaderSection id="Startup Opportunities" title="Startup Opportunities"><ReaderOpportunities items={analysis?.startupOpportunities} /></ReaderSection>
      <ReaderSection id="Product Ideas" title="Product Ideas"><ReaderOpportunities items={analysis?.productIdeas} /></ReaderSection>
...
function ReaderBullets({ items }: { items?: string[] }) { return items?.length ? <ul className="space-y-2">{items.map((item) => <li key={item} className="flex gap-2"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-lime" /><span>{item}</span></li>)}</ul> : <p>{fallback()}</p>; }
function ReaderPoints({ items }: { items?: BriefPoint[] }) { return items?.length ? <div className="grid gap-3 md:grid-cols-2">{items.map((item, index) => <ReaderCard key={`${item.title}-${index}`} title={item.title} detail={item.detail} />)}</div> : <p>{fallback()}</p>; }
function ReaderOpportunities({ items }: { items?: BriefOpportunity[] }) { return items?.length ? <div className="grid gap-3 md:grid-cols-2">{items.map((item) => <ReaderCard key={item.title} title={`${item.title} (${item.confidence} confidence)`} detail={item.detail} />)}</div> : <p>{fallback()}</p>; }
